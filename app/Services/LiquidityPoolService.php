...
final class LiquidityPoolService {
    ...
    public function buyBtc($t,$s,$q,$p):array{if($q<=0)throw new DomainException('Quantidade inválida.');$cost=$q*(float)$s['btc_buy_price'];if((float)$t['cash_balance']<$cost)throw new DomainException('Caixa insuficiente.');return ['cash'=>-$cost,'btc'=>$q,'nft'=>0,'share'=>0,'poolN'=>(int)$p['pool_nfts'],'tot'=>(int)$p['total_shares'],'msg'=>'Time '.$t['name'].' comprou '.rtrim(rtrim(number_format($q,2,'.',''),'0'),'.').' BTC por R$ '.number_format($cost,2,',','.').'.'];}
    public function sellBtc($t,$s,$q,$p):array{if($q<=0)throw new DomainException('Quantidade inválida.');if((float)$t['btc_balance']<$q)throw new DomainException('Saldo de BTC insuficiente.');$value=$q*(float)$s['btc_sell_price'];return ['cash'=>$value,'btc'=>-$q,'nft'=>0,'share'=>0,'poolN'=>(int)$p['pool_nfts'],'tot'=>(int)$p['total_shares'],'msg'=>'Time '.$t['name'].' vendeu '.rtrim(rtrim(number_format($q,2,'.',''),'0'),'.').' BTC por R$ '.number_format($value,2,',','.').'.'];}
    public function sellNft($t,$s,$p):array{if((int)$t['nft_balance']<1)throw new DomainException('Este time não possui NFT em mãos para vender.');return ['cash'=>(float)$s['nft_sell_price'],'btc'=>0,'nft'=>-1,'share'=>0,'poolN'=>(int)$p['pool_nfts'],'tot'=>(int)$p['total_shares'],'msg'=>'Time '.$t['name'].' vendeu 1 NFT em mãos por R$ '.number_format((float)$s['nft_sell_price'],2,',','.').'.'];}
    public function sellShare($t,$s,$p):array{if((int)$t['pool_shares']<1)throw new DomainException('Este time não possui cota para vender.');if((int)$p['total_shares']<1)throw new DomainException('A piscina não possui cotas para reduzir.');return ['cash'=>(float)$s['share_sell_price'],'btc'=>0,'nft'=>0,'share'=>-1,'poolN'=>(int)$p['pool_nfts'],'tot'=>(int)$p['total_shares']-1,'msg'=>'Time '.$t['name'].' vendeu 1 cota da piscina por R$ '.number_format((float)$s['share_sell_price'],2,',','.').'.'];}
