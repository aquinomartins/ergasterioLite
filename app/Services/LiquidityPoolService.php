...
final class LiquidityPoolService {
    ...
    public function withdrawNftWithCash($t,$s,$p):array{if((float)$t['cash_balance']<(float)$s['cash_withdraw_cost'])throw new DomainException('Caixa insuficiente para retirar NFT com dinheiro.');if((int)$p['pool_nfts']<1)throw new DomainException('A piscina não possui NFT disponível para retirada.');return ['cash'=>-(float)$s['cash_withdraw_cost'],'btc'=>0,'nft'=>1,'share'=>0,'poolN'=>(int)$p['pool_nfts']-1,'tot'=>(int)$p['total_shares'],'msg'=>'Equipe '.$t['name'].' retirou 1 NFT da piscina pagando R$ '.number_format((float)$s['cash_withdraw_cost'],2,',','.').'.'];}
    public function buyBtc($t,$s,$q,$p):array{if($q<=0)throw new DomainException('A quantidade deve ser maior que zero.');$cost=$q*(float)$s['btc_buy_price'];if((float)$t['cash_balance']<$cost)throw new DomainException('Caixa insuficiente para comprar BTC.');return ['cash'=>-$cost,'btc'=>$q,'nft'=>0,'share'=>0,'poolN'=>(int)$p['pool_nfts'],'tot'=>(int)$p['total_shares'],'msg'=>'Equipe '.$t['name'].' comprou '.rtrim(rtrim(number_format($q,2,'.',''),'0'),'.').' BTC por R$ '.number_format($cost,2,',','.').'.'];}
    public function sellBtc($t,$s,$q,$p):array{if($q<=0)throw new DomainException('A quantidade deve ser maior que zero.');if((float)$t['btc_balance']<$q)throw new DomainException('BTC insuficiente para venda.');$value=$q*(float)$s['btc_sell_price'];return ['cash'=>$value,'btc'=>-$q,'nft'=>0,'share'=>0,'poolN'=>(int)$p['pool_nfts'],'tot'=>(int)$p['total_shares'],'msg'=>'Equipe '.$t['name'].' vendeu '.rtrim(rtrim(number_format($q,2,'.',''),'0'),'.').' BTC por R$ '.number_format($value,2,',','.').'.'];}
    public function sellNft($t,$s,$p):array{if((int)$t['nft_balance']<1)throw new DomainException('Sem NFT em mãos para venda.');return ['cash'=>(float)$s['nft_sell_price'],'btc'=>0,'nft'=>-1,'share'=>0,'poolN'=>(int)$p['pool_nfts'],'tot'=>(int)$p['total_shares'],'msg'=>'Equipe '.$t['name'].' vendeu 1 NFT em mãos por R$ '.number_format((float)$s['nft_sell_price'],2,',','.').'.'];}
    public function sellShare($t,$s,$p):array{if((int)$t['pool_shares']<1)throw new DomainException('Sem cota da piscina para venda.');if((int)$p['total_shares']<1)throw new DomainException('A piscina não possui cotas para reduzir.');return ['cash'=>(float)$s['share_sell_price'],'btc'=>0,'nft'=>0,'share'=>-1,'poolN'=>(int)$p['pool_nfts'],'tot'=>(int)$p['total_shares']-1,'msg'=>'Equipe '.$t['name'].' vendeu 1 cota da piscina por R$ '.number_format((float)$s['share_sell_price'],2,',','.').'.'];}
