...
<div class="liquidity-dashboard">
    <section class="liquidity-header">
        ...
        <p>Código: <strong><?= $e($s['access_code'] ?? '-') ?></strong> · Rodada <?= (int)($s['current_round'] ?? 0) ?>/<?= (int)($s['total_rounds'] ?? 0) ?> · Fase: <?= $e($s['session_phase'] ?? 'regular') ?> · Status: <?= $e($s['status'] ?? '-') ?></p>
